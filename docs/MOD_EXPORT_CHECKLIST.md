...
- [ ] **Starting traits** — Planned. Capture a TGSRR-owned starting snapshot using stable trait IDs.
- [ ] **Current traits** — Planned. Export a current stable-ID snapshot; the website derives additions and removals.
...
- [ ] **Kills with each weapon** — Planned. Requires TGSRR-owned kill attribution keyed by full item type.
- [x] **Current level and XP for every skill** — Implemented. Current skill state already powers the Tracker.
- [ ] **Daily XP gained for every skill** — Planned. Aggregate `AddXP` by stable perk ID and seal it at the day boundary.
...
- [x] **Completed outposts** — Implemented. Whole-outpost completion rising-edge events already exist.
- [x] **Kill milestones** — Implemented. Persistent claims include world-age timestamps.
- [ ] **Skill milestones** — Planned. Generic level-reached events exist; milestone policy and persistent records remain unfinished.
- [x] **Outpost milestones** — Implemented. Deliverable and whole-outpost completion claims are persisted.
...
- [x] **Zombies killed** — Implemented through the agreed total-kills record.
- [ ] **Kills by weapon class** — Planned. Prefer deriving website classes from exported full weapon IDs unless runtime classification carries otherwise unavailable meaning.
- [ ] **Kills by specific weapon** — Planned through the agreed per-weapon kill collector.
- [ ] **Trait population comparisons** — Planned. TGSRR exports stable starting/current IDs; the website performs cross-run comparisons.
...
- [ ] **Total experience gained** — Planned. Derive from TGSRR's per-skill XP aggregates without duplicating raw XP events.
- [x] **Outpost completion order** — Implemented foundation. Derive ordering from existing whole-outpost completion timestamps; final export projection remains to be written.
- [x] **Days elapsed when each milestone is reached** — Implemented foundation. Claims already retain world age; final export projection remains to be written.
- [ ] **Days elapsed when each skill reaches level 10** — Planned. Persist the existing level-reached edge under the final skill milestone policy.
