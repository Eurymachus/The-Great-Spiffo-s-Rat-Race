...
| Contract | Save rollbacks and recovery decisions | 🟡 Partial | Crash-interrupted session commits are automatically reconciled only when the verified ahead tail contains `session.started` and existing recovery-evidence records and the independent session-log cursor agrees. TGSRR adopts the immutable tail and appends `run.recovery.decided` with both cursors and structured decision authority `{ type = "system", id = "tgsrr" }`. Ahead gameplay events remain halted pending preserved-branch recovery; eligibility consequences belong to the website/moderator. |
