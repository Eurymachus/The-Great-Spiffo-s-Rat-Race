...
| Contract | Starting location | 🟢 Implemented | Run creation captures immutable integer XYZ, the precision-safe BuildingDef ID when present, UTC, and world age in `character.startingLocation`. Existing-save observations are marked partial and do not claim to be the original spawn. Town and spawn-region interpretation belongs to the website. |
