...
| Team request | Weapons broken | 🟢 Implemented | TGSRR follows the Build 42 dual path established by Twist Stats: wrap weapon `OnBreak` callbacks and briefly observe condition after `OnWeaponSwing` for weapons without a callback. Weak item-reference deduplication prevents both paths counting the same break. Each local-player break appends `weapon.broken` with the original full item ID; format-3 exports cumulative ID-sorted totals and active-day deltas. Existing runs are partial. |
