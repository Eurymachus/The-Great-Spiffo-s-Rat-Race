...
| Team request | Animals petted | 🟡 Planned | Count each successfully completed local-player `ISPetAnimal` interaction and aggregate by PZ's raw animal type. This measures completed petting actions, not whether the animal or player received a cooldown-limited mechanical benefit: `IsoAnimal:petAnimal()` still calls `IsoPlayer:petAnimal()` when the animal's own pet timer blocks stress, acceptance, or Husbandry gains, and the player's benefit has a separate one-hour calendar cooldown. Individual pets will not be ledger events; bootstrapped runs will disclose partial history. |
| Team request | Fluid consumed by type | 🟡 Planned | Measure actual fluid removed for the local player and aggregate cumulative litres by canonical raw fluid type, with an explicit `Mixed` classification unless Build 42.19 exposes exact per-component removal. Coverage must include manual container drinking, direct world-source drinking, partial/cancelled actions, and Java `autoDrink()`. `Hook.AutoDrink` is a cancellation hook rather than a passive notification, so TGSRR must not register it merely to observe consumption. The planned low-cost auto-drink collector correlates the selected eligible water-container amount with the authoritative thirst reduction and excludes already-attributed manual drink actions; it does not continuously scan every inventory item. Individual sips will not be ledger events; bootstrapped runs will disclose partial history. |
| Team request | Calories consumed | 🟡 Planned | Accumulate calories actually applied by completed or partially applied local-player food and fluid consumption. Build 42.19 applies food calories in `IsoGameCharacter:Eat()` and fluid calories in `IsoGameCharacter:DrinkFluid()`; burnt food contributes one fifth of its stated calories. The collector must measure consumed input rather than net Nutrition calorie-balance change, because metabolism runs continuously and the Nutrition balance is clamped. Aggregate calories only; individual meals will not be ledger events and bootstrapped runs will be partial. |
...
| Team request | Generator repairs | 🟡 Planned | Count each successful local-player `ISFixGenerator:complete()` action, meaning one electronics scrap was consumed, and accumulate the generator's actual before/after condition restored. One menu command may queue several repairs, so each completed scrap-consuming action counts separately. The nominal gain is `4 + Electrical level / 2`, but the exported restoration total must use the authoritative clamped condition delta at the final repair. Individual repairs will not be ledger events; bootstrapped runs will disclose partial history. |
