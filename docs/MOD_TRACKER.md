...
The intended player feedback sequence is `Approaching/Proximity`, `Clearing`, `Remaining Zombies`, `Cleared`, then `Completed`. Proximity is informational; Cleared and Completed retain the definitions in [MOD_OUTPOSTS.md](MOD_OUTPOSTS.md).
...
- Whether the common deliverable schema needs extension for final systems.
