...
- Food containers are discovered after every registered ground-floor room has a populated live `IsoRoom`, then maintained from object-add/remove events. The compact registered outposts fit within the loaded-cell radius while active. This avoids comparing `IsoRoom` square counts with `RoomDef:getArea()`, whose map-rectangle area can include voids and permanently reject a fully loaded room. Only cached container contents are totalled once per second while the outpost is active. Streaming loss invalidates the object-reference cache without overwriting the last persisted result; the cache and live result are rebuilt when the rooms stream again.
...
- Bare wooden and metal wall frames do not seal an exterior-envelope segment. The enclosure scan recognizes the canonical Build 42 wall-frame entity sprites and the legacy converted wooden-frame object name before classifying an object returned by `IsoGridSquare:getWall()` as solid.
