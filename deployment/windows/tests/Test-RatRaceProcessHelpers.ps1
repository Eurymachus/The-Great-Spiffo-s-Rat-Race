...
$processes = @(
    [pscustomobject]@{ ProcessId = 10; ParentProcessId = 1; CommandLine = 'powershell -File "G:\RatRace\_Staging\releases\abc\deployment\windows\Start-RatRaceScheduledProcess.ps1" -Process Web' }
    [pscustomobject]@{ ProcessId = 11; ParentProcessId = 10; CommandLine = 'powershell -File "G:\RatRace\_Staging\releases\abc\deployment\windows\Start-RatRaceProcess.ps1" -Process Web' }
    [pscustomobject]@{ ProcessId = 12; ParentProcessId = 11; CommandLine = 'python -m uvicorn config.asgi:application' }
    [pscustomobject]@{ ProcessId = 13; ParentProcessId = 1; CommandLine = '"G:\RatRace\_Staging\venv\Scripts\python.exe" -m uvicorn config.asgi:application' }
...
$roots = @(Get-RatRaceDeploymentProcessRoots `
    -Processes $processes `
    -InstallationRoot 'G:\RatRace\_Staging' `
    -PythonExecutable 'G:\RatRace\_Staging\venv\Scripts\python.exe' `
    -Process Web)
...
Assert-Equal $true (Test-RatRacePathScope 'G:\RatRace\_Staging\logs' 'G:\RatRace\_Staging') "A child path should be accepted."
Assert-Equal $false (Test-RatRacePathScope 'G:\RatRace\logs' 'G:\RatRace\_Staging') "Production should be rejected from staging scope."
