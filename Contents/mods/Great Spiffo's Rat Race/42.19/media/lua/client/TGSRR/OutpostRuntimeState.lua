...
function Runtime.evaluate(outpost, player)
    ...
    if activationAvailable then
        Store.updateDeliverable(outpost.id, "room_activation", {
            passed = activation.passed,
            current = activation.activatedRooms,
            required = activation.totalRooms,
        })
        Store.updateDeliverable(outpost.id, "floor_activation", {
            passed = activation.passed,
    ...
    if zombies ~= nil then
        ...
        Store.updateDeliverable(outpost.id, "zombie_clearance", {
    ...
    if windows and windows.available == true then
        Store.updateDeliverable(outpost.id, "window_barricades", windows)
    end
    if enclosed and enclosed.available == true then
        Store.updateDeliverable(outpost.id, "enclosed", enclosed)
    end
    if doorsFitted and doorsFitted.available == true then
        Store.updateDeliverable(outpost.id, "doors_fitted", doorsFitted)
    end
    if doorsClosed and doorsClosed.available == true then
        Store.updateDeliverable(outpost.id, "doors_closed", doorsClosed)
    end
    return Store.get(outpost.id)
end
...
function Runtime.getStatus(id)
    ...
    -- Complete remains a live derived state once every deliverable check exists.
    return "discovered"
end
...
local function updateActiveOutpost(player)
    ...
    if outpost ~= activeOutpost then
        ...
        if outpost then markDiscovered(outpost) end
    end
    return outpost
end
