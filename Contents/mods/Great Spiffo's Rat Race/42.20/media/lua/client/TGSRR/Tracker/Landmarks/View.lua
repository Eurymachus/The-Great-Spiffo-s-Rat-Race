...
local function landmarkRight(width)
    return math.min(STATUS_COLUMN_WIDTH, math.floor((width - 100) / 2))
end

local function statusLeft(width)
    return width - landmarkRight(width)
end
...
function View:prerender()
    ...
    self:drawRect(MARGIN, HEADER_Y, frameWidth, HEADER_HEIGHT,
        0.9, 0.12, 0.12, 0.12)
    self:drawRectBorder(MARGIN, HEADER_Y, frameWidth, HEADER_HEIGHT,
        0.7, 0.65, 0.65, 0.65)
    local textY = HEADER_Y + math.floor(
        (HEADER_HEIGHT - getTextManager():getFontHeight(UIFont.Small)) / 2)
    self:drawText(L.text("UI_TGSRR_Tracker_Landmark", "Landmark"),
        MARGIN + 8, textY, 1, 1, 1, 1, UIFont.Small)
    local landmarkX = landmarkRight(frameWidth)
    local statusX = statusLeft(frameWidth)
    ...
    self:drawRect(MARGIN, footerY, frameWidth, FOOTER_HEIGHT,
        0.8, 0.02, 0.02, 0.02)
    local total = self.total or 0
    local visited = self.visited or 0
    if total > 0 and visited > 0 then
        self:drawRect(MARGIN + 1, footerY + 1,
            math.floor((frameWidth - 2) * visited / total), FOOTER_HEIGHT - 2,
            0.76, 0.12, 0.58, 0.18)
    end
    self:drawRectBorder(MARGIN, footerY, frameWidth, FOOTER_HEIGHT,
        0.62, 0.55, 0.55, 0.55)
    self:drawTextCentre(tostring(visited) .. " "
        ...
        MARGIN + frameWidth / 2,
        footerY + math.floor((FOOTER_HEIGHT
...
function View:drawLandmark(y, item, alt)
    ...
    local landmarkX = landmarkRight(width)
    local statusX = statusLeft(width)
    ...
    self:drawTextCentre(row.visited
        ...
        statusX + math.floor((width - statusX) / 2), textY,
        color[1], color[2], color[3], 1, UIFont.Small)
